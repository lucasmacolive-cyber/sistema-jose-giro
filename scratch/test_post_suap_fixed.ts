...
async function testPostFixed() {
  ...
  const loginResp = await request("POST", "/accounts/login/", jar, loginBody, {
    ...
    "X-CSRFToken": csrfLogin,
  });

  console.log("Login status:", loginResp.status, "Location:", loginResp.headers.location);
  ...
  console.log("Report CSRF:", reportCsrf);

  // Convert query parameters into POST body parameters as well as keeping them in URL
  const urlObj = new URL(SUAP_BASE + SUAP_RELATORIO_URL);
  const postParams = new URLSearchParams(urlObj.searchParams);
  postParams.set("csrfmiddlewaretoken", reportCsrf);
  postParams.set("xls", "1");

  console.log("4. POST export xls...");
  const exportResp = await request("POST", SUAP_RELATORIO_URL + "&xls=1", jar, postParams.toString(), {
  ...
  console.log("Export location:", exportResp.headers.location);
  console.log("Export content-type:", exportResp.headers["content-type"]);
  console.log("Export text snippet:", exportResp.text.slice(0, 500));

  fs.writeFileSync("scratch/fixed_export_post.html", exportResp.text);
}

testPostFixed().catch(console.error);
