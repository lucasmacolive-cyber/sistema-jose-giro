...
async function dumpHtml() {
  ...
  fs.writeFileSync("scratch/suap_export_post.html", exportResp.text);

  const links = [...exportResp.text.matchAll(/href="([^"]+)"/g)].map(m => m[1]);
  console.log("Links found in response:", links.filter(l => l.includes("djtools") || l.includes("relatorio") || l.includes("xls") || l.includes("download")));

  const forms = [...exportResp.text.matchAll(/action="([^"]+)"/g)].map(m => m[1]);
  console.log("Forms found in response:", forms);
}
